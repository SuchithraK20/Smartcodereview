...
- Changed lines are marked with [CHANGED] at the end of the line
- You MUST use the EXACT line number shown at the start of the line in your response
- DO NOT comment on lines that are not marked as [CHANGED]
- Focus ONLY on lines marked as [CHANGED] and provide specific feedback for each line (e.g., readability, security, performance, etc.)
...
- Be specific in your suggestions. Explain what is wrong with the current implementation and propose a better approach.
...
Rules:
1. Only comment on lines marked with [CHANGED].
2. Use EXACT line numbers shown at the start of the line.
3. Be specific and actionable in your recommendations.
4. If no issues are found in the changed lines, return an empty array.
"""
